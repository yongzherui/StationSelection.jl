...
@testset "Exact DARP Route Column Generation" begin
    function make_data(
        stations::Vector{Int},
        requests::DataFrame;
    ...
    function make_model(; vehicle_capacity::Int=2, max_route_length::Int=3)
        return StationSelection.ExactDARPRouteModel(
            2, 2;
    ...
    function make_state(model, data)
        ...
        return StationSelection.ExactDARPRouteColumnGenerationState(
    ...
    function make_duals(entries::Vector{Tuple{Int, Int, Int, Int, Float64}})
        return StationSelection.ExactDARPRouteCGDuals(
    ...
    @testset "station with no outgoing demand is still visited for dropoff" begin
        ...
        base = StationSelection._build_exact_darp_route_base(model, data)
        qbar = StationSelection.compute_qbar(data.scenarios[1], base.valid_jk_pairs, model.time_window_sec)
        pricing_data = StationSelection.ExactDARPRouteBucketPricingData(
            1,
            0,
            [1, 2, 3],
            StationSelection.ExactDARPRouteBucketDemandCaps(1, 0, qbar[0].caps),
            Dict((1, 2) => 10.0),
        ...
        label = StationSelection.ExactDARPRoutePricingLabel(
            1,
            BitSet([1]),
            [1],
            0.0,
    ...
    @testset "pricing accepts normalized negative master duals" begin
        ...
        pricing = StationSelection.solve_exact_darp_route_pricing(
            model,
            data,
            state,
            StationSelection.ExactDARPRouteCGDuals(
                Dict((1, 0, 1, 2) => 50.0),
                Dict((1, 0, 1, 2) => -50.0),
            );
            max_columns=10,
    ...
    @testset "restricted master build and runner smoke" begin
        if !gurobi_available
            ...
            build_result = StationSelection.build_exact_darp_route_restricted_master(
                model,
                data,
                route_pool,
            )
            ...
            config = StationSelection.ExactDARPRouteColumnGenerationConfig(
                max_iterations=2,
                pricing_time_limit_sec=5.0,
            )
            runner_result = StationSelection.run_exact_darp_route_column_generation(
                model,
                data,
                config;
                silent=true,
            ...
            @test haskey(runner_result.final_result.metadata, "exact_darp_route_column_generation")
