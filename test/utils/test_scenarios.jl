...
@testset "GenerateScenarios Tests" begin
    ...
    @testset "1-hour segments" begin
        scenarios = generate_scenarios(Date(2025,6,1), Date(2025,6,2); segment_hours=1)
    ...
    @testset "6-hour segments" begin
        scenarios = generate_scenarios(Date(2025,6,1), Date(2025,6,3); segment_hours=6)
    ...
    @testset "weekly cycle 24-hour" begin
        scenarios = generate_scenarios(Date(2025,6,1), Date(2025,6,30); segment_hours=24, weekly_cycle=true)
    ...
    @testset "weekly cycle 4-hour" begin
        scenarios = generate_scenarios(Date(2025,6,1), Date(2025,6,14); segment_hours=4, weekly_cycle=true)
