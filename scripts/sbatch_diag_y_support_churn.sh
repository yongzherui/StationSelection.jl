#!/bin/bash
# Per-iteration y_j support churn across CG, for 4 cases that certify under
# uncapped max_stops (so the trace is complete through certification):
#   0: n25 sc1 s42   (uncapped optimum = one long 7-stop route)
#   1: n20 sc3 s42
#   2: n30 sc3 s43
#   3: n30 sc1 s42   (control: uncapped == ms4)
#SBATCH --job-name=pfa_ychurn
...
OUTDIR="${PFAYC_OUTDIR:?set PFAYC_OUTDIR}"; mkdir -p "$OUTDIR"
...
stdbuf -oL -eL julia --startup-file=no --color=no --project="$PROJECT_ROOT" \
  "$PROJECT_ROOT/scripts/diag_y_support_churn.jl" "$N" \
  > "$OUTDIR/ychurn_n${N}_sc${S}_s${SEED}.txt" 2>&1
echo "wrote $OUTDIR/ychurn_n${N}_sc${S}_s${SEED}.txt"
