#!/bin/bash
#SBATCH --job-name=sample09_ww_task
...
# sample_09 walk_cost_weight sensitivity (BendersY mw, route_reg_weight fixed
# at 1.0) -- SLURM array task runner. Each array task reads ONE line from the
# job list (one (n_stations, walk_cost_weight) pair) and runs it via
# sample09_walk_weight_task.jl, so every combination lands on its own node.
# Submitted via submit_sample09_walk_weight_task.sh.
#
# Usage (via submit_sample09_walk_weight_task.sh):
#   sbatch --array=1-N --output=... --error=... \
#          scripts/sbatch_sample09_walk_weight_task.sh <jobs_file> <base_outdir>
...
if [ -z "$JOBS_FILE" ] || [ -z "$BASE_OUTDIR" ]; then
    echo "ERROR: Usage: sbatch_sample09_walk_weight_task.sh <jobs_file> <base_outdir>"
    exit 1
fi
if [ -z "$TASK" ]; then
...
WALK_WEIGHT=$(echo  "$JOB_LINE" | cut -f2)

echo "=========================================="
echo "sample_09 walk_cost_weight sensitivity - array task"
echo "Array job:  ${SLURM_ARRAY_JOB_ID}  task: ${TASK}"
echo "n_stations: ${N_STATIONS}   walk_cost_weight: ${WALK_WEIGHT}   route_reg_weight: 1.0 (fixed)"
...
stdbuf -o0 -e0 julia --startup-file=no \
      ...
      "$PROJECT_ROOT/scripts/sample09_walk_weight_task.jl" \
      "$BASE_OUTDIR" "$N_STATIONS" "$WALK_WEIGHT"
EXIT_CODE=$?
set -e
