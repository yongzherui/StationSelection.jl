#!/bin/bash
#SBATCH --job-name=sample09_rw_task
...
# sample_09 route_reg_weight sensitivity (BendersY mw) -- SLURM array task
# runner. Each array task reads ONE line from the job list (one
# (n_stations, route_reg_weight) pair) and runs it via
# sample09_route_weight_task.jl, so every combination lands on its own node.
# Submitted via submit_sample09_route_weight_task.sh.
#
# Usage (via submit_sample09_route_weight_task.sh):
#   sbatch --array=1-N --output=... --error=... \
#          scripts/sbatch_sample09_route_weight_task.sh <jobs_file> <base_outdir>
...
if [ -z "$JOBS_FILE" ] || [ -z "$BASE_OUTDIR" ]; then
    echo "ERROR: Usage: sbatch_sample09_route_weight_task.sh <jobs_file> <base_outdir>"
    exit 1
fi
if [ -z "$TASK" ]; then
...
ROUTE_WEIGHT=$(echo  "$JOB_LINE" | cut -f2)

echo "=========================================="
echo "sample_09 route_reg_weight sensitivity - array task"
echo "Array job:  ${SLURM_ARRAY_JOB_ID}  task: ${TASK}"
echo "n_stations: ${N_STATIONS}   route_reg_weight: ${ROUTE_WEIGHT}"
...
stdbuf -o0 -e0 julia --startup-file=no \
      ...
      "$PROJECT_ROOT/scripts/sample09_route_weight_task.jl" \
      "$BASE_OUTDIR" "$N_STATIONS" "$ROUTE_WEIGHT"
EXIT_CODE=$?
set -e
