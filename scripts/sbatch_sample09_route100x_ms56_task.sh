#!/bin/bash
#SBATCH --job-name=sample09_route100x_ms56
...
# sample_09 route100x/ms5-ms6 -- SLURM array task runner. Each array task reads
# ONE line from the job list (one (n_stations, method) pair) and runs it via
# sample09_route100x_ms56_task.jl, so every combination lands on its own node
# instead of running sequentially in one job. Submitted via
# submit_sample09_route100x_ms56_task.sh.
#
# Usage:
#   sbatch --array=1-N --output=... --error=... \
#          scripts/sbatch_sample09_route100x_ms56_task.sh <jobs_file> <base_outdir>
...
if [ -z "$JOBS_FILE" ] || [ -z "$BASE_OUTDIR" ]; then
    echo "ERROR: Usage: sbatch_sample09_route100x_ms56_task.sh <jobs_file> <base_outdir>"
    exit 1
fi
if [ -z "$TASK" ]; then
...
echo "=========================================="
echo "sample_09 route100x/ms5-ms6 - array task"
...
stdbuf -o0 -e0 julia --startup-file=no \
      ...
      "$PROJECT_ROOT/scripts/sample09_route100x_ms56_task.jl" \
