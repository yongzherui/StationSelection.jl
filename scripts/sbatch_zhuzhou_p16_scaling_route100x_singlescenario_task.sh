#!/bin/bash
#SBATCH --job-name=zz_p16_scaling_ss
...
# zhuzhou p16 route100x scaling study, SINGLE-SCENARIO twin of
# sbatch_zhuzhou_p16_scaling_route100x_task.sh -- same array-task-per-row
# structure, pointed at zhuzhou_p16_scaling_route100x_singlescenario_task.jl
# instead. Same 32G/12h budget as the 3-scenario batches (n_scenarios=1 should
# only make each job cheaper, never more expensive, so no need to shrink the
# allocation).
#
# Usage:
#   sbatch --array=1-N --output=... --error=... \
#          scripts/sbatch_zhuzhou_p16_scaling_route100x_singlescenario_task.sh <jobs_file> <base_outdir>
...
if [ -z "$JOBS_FILE" ] || [ -z "$BASE_OUTDIR" ]; then
    echo "ERROR: Usage: sbatch_zhuzhou_p16_scaling_route100x_singlescenario_task.sh <jobs_file> <base_outdir>"
    exit 1
fi
if [ -z "$TASK" ]; then
...
echo "=========================================="
echo "zhuzhou p16 route100x scaling (single-scenario) - array task"
...
stdbuf -o0 -e0 julia --startup-file=no \
      ...
      "$PROJECT_ROOT/scripts/zhuzhou_p16_scaling_route100x_singlescenario_task.jl" \
