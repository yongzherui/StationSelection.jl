...
#          scripts/sbatch_zhuzhou_assignment.sh <jobs_file> <base_outdir> <data_dir>
...
if [ -z "$JOBS_FILE" ] || [ -z "$BASE_OUTDIR" ] || [ -z "$DATA_DIR" ]; then
    echo "ERROR: Usage: sbatch_zhuzhou_assignment.sh <jobs_file> <base_outdir> <data_dir>"
    exit 1
fi
if [ -z "$TASK" ]; then
...
export CS_N_SCENARIOS="${CS_N_SCENARIOS:-1}"
