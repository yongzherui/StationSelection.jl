#!/bin/bash
#SBATCH --job-name=ss_pricing_tests
...
# Copy the depot per-job so concurrent runs don't race on precompilation.
...
cd "$PROJECT_ROOT"
julia --startup-file=no --project="$PROJECT_ROOT" "$WORKDIR/run_ss_tests.jl"
