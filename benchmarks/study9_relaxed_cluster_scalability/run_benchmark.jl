...
# or off and these two columns must stay `false`. That keeps one schema across rows written
# before and after the change -- this study is mid-flight.
#
# The assertion messages below used to say the features "no longer exist", which asserted
# the opposite of the truth and would send a reader looking for code that is right there.
!barren_cache ||
...
# Late-stage dual snapshots, opt-in via STUDY9_EXPORT_DUALS=1.
#
...
export_duals = get(ENV, "STUDY9_EXPORT_DUALS", "0") == "1"
