#!/bin/bash
# Request-level structure probe: what do the 16 trip requests look like, easy vs hard seed.
# Dumps the raw pairs and describes each scenario as a directed graph. No solve.
#SBATCH --job-name=req_level
...
#SBATCH -o /home/yongzr/2025-09-JacqWang-Microtransit/StationSelection.jl/slurm_logs/req-level-%j.out
#SBATCH -e /home/yongzr/2025-09-JacqWang-Microtransit/StationSelection.jl/slurm_logs/req-level-%j.err
set -euo pipefail
...
stdbuf -oL -eL julia --startup-file=no --color=no --project="$PROJECT_ROOT" \
    benchmarks/diagnostics/request_level_structure.jl
