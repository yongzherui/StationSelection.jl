...
- `alpha_profile_hint::Union{Dict{NTuple{3,Int},Float64},Nothing}`: Optional alpha warm-start
  hints keyed by `(route_id, pickup_station_id, dropoff_station_id)`. Set when routes are
...
function create_vehicle_capacity_od_map(
    ...
    if !isnothing(model.routes_file)
        ...
        # keep routes whose detour_feasible_legs includes at least one leg in jk_sids.
        for s in 1:S
            routes_s[s] = Dict{Int, Vector{RouteData}}()
            for t_id in sort(collect(keys(Q_s_t[s])))
                # Convert valid (j_idx, k_idx) pairs to station-ID pairs for this bucket
                ...
                # A route is relevant if any of its detour_feasible_legs is in jk_sids
