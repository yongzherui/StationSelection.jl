"""
OD mapping for ExactDARPRouteModel.
...
struct ExactDARPRouteODMap <: AbstractClusteringMap
...
has_walking_distance_limit(mapping::ExactDARPRouteODMap) = true

get_valid_jk_pairs(mapping::ExactDARPRouteODMap, o::Int, d::Int) =
    get(mapping.valid_jk_pairs, (o, d), Tuple{Int,Int}[])

_time_ids(mapping::ExactDARPRouteODMap, s::Int) =
    sort!(collect(keys(mapping.Q_s_t[s])))

_time_od_pairs(mapping::ExactDARPRouteODMap, s::Int, t_id::Int) =
    sort!(collect(keys(mapping.Q_s_t[s][t_id])))

function _build_exact_darp_route_base(
...
function _collect_alpha_profile(global_state::ExactDARPRouteBucketPoolsState)
    alpha_profile = Dict{NTuple{3, Int}, Float64}()
...
function _legacy_exact_darp_route_init_spec(model::ExactDARPRouteModel)::RoutePoolInitSpec
...
function create_exact_darp_route_od_map(
    model::ExactDARPRouteModel,
    data::StationSelectionData,
    bucket_pools::ExactDARPRouteBucketPoolsState
)::ExactDARPRouteODMap
...
function create_exact_darp_route_od_map(
    ...
    bucket_pools = initialize_route_pool(
        _legacy_exact_darp_route_init_spec(model),
        data,
        base.Q_s_t,
        base.valid_jk_pairs;
    ...
    return create_exact_darp_route_od_map(model, data, bucket_pools)
end
