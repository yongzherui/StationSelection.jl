...
`passenger_free_assignment/types.jl` for each pricer's own data/label/filters
...
`passenger_free_assignment`'s). The bucket is kept sorted by
...
`PassengerFreeAssignmentSearchContext`, `PassengerFreeAssignmentStationSimpleSearchContext`)
...
itself. Only `PassengerFreeAssignmentSearchContext` overrides it today, for
...
`PassengerFreeAssignmentSearchContext` overrides it, stashing both into mutable
