...
module ClusteringTwoStageLambda
...
function clustering_two_stage_lambda(
    ...
    @objective(m, Min, sum(scenario_request_counts[s][idx_to_id[i]] * costs[(idx_to_id[i], idx_to_id[j])] * x[i,j,s] for i in 1:n, j in 1:n, s in 1:S) + lambda * sum(z[j,s] for j in 1:n, s in 1:S))
    ...
    # This constraint ensures that we select exactly k medoids
