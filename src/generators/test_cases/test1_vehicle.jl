...
CLAUDE.md do not exist in the current codebase; `ExactDARPRouteModel`, which
previously had a `vehicle_capacity` field, was removed as an ineffective
benchmark). The capacity sub-sweep (fleet fixed at 2, capacity 20→5) is still
generated by `T1_FLEET_CONFIGS`/`build_test1_cases`, but there is currently
no model in `src/` that checks the capacity hypothesis against it.
"""
