"""
Generic label-setting engine shared by every pricer: the sparse station-age
dominance mechanics, the dominance-bucket entry/container, the single-walk
bucket-insertion primitive, and the priority-queue search context/loop. See
`aggregate_od_route/types.jl` / `joint_routing_assignment/types.jl` for each
pricer's own data/label/filters types that plug into the generics here.
"""

# ── Sparse station-age mechanics ────────────────────────────────────────────
# Reward-model-independent math shared by the aggregate and PFA pricers.
