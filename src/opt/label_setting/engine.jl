"""
The label-setting search loop itself. See `types.jl` for the dominance-bucket
container and the `AbstractPricingSearchContext` hook contract this loop is
...
function _run_pricing_label_search(
    ...
    # (live_labels, n_live_labels, dominance_buckets, frontier, next_label_id,
