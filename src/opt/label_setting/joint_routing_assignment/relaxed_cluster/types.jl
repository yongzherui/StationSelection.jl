...
label rather than being applied as a filter afterwards, and `seed.jl` /
`extend.jl` / `context.jl` / `hooks.jl` for the label-setting
functionality built on the type below.

There is no `dominate.jl`: this pricer reuses `../exact/`'s
...
itself carries the mask (`hooks.jl`), so labels that have escaped different
...
Drop the mask. `seed.jl`/`extend.jl` project through this so the aging,
