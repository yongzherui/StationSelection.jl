...
double count. See the module docstring in `exact/types.jl` for the label-level
...
"OR their prefix masks", per the design note in `exact/types.jl`: since masks are
