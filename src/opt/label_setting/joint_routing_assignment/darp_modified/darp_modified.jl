"""
`JointRoutingAssignmentDarpModifiedSearchContext`: `darp_modified/`'s plug into the shared
...
`AggregateODRouteJointRoutingAssignmentFormulation.pricing_mode === :darp_modified` --
...
`joint_routing_assignment_pricing_by_darp_modified_label_setting` below -- the same
