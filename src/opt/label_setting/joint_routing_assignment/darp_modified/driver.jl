...
`joint_routing_assignment_pricing_by_darp_modified_label_setting` below is
the same driver shape the now-removed pre-hub driver functions
(`aggregate_od_route_pricing_by_label_setting` and friends) used. Exists so
`darp_modified/` can be benchmarked/compared against `exact/` directly (no
live master model/duals needed for a bare comparison run), not as a
production entrypoint -- `AggregateODRouteJointRoutingAssignmentFormulation`'s
`CGSolver` build always goes through `round.jl`.
"""

export joint_routing_assignment_pricing_by_darp_modified_label_setting

"""
    joint_routing_assignment_pricing_by_darp_modified_label_setting(pricing_data, existing_columns;
...
comparison run) so `darp_modified/` can be benchmarked against `exact/` directly, the
same shape the pre-hub driver functions (`aggregate_od_route_pricing_by_label_setting`
and friends) used.
"""
function joint_routing_assignment_pricing_by_darp_modified_label_setting(
    ...
    ctx = JointRoutingAssignmentDarpModifiedSearchContext(pricing_data)
