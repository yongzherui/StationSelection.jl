"""
Label/bitsets/dominance/pricing-data types for `darp_modified/`: a *provably
...
cutoff), `darp_modified/`'s optimal reduced cost for a scenario must equal `exact/`'s,
and the two `CGSolver` modes (`AggregateODRouteJointRoutingAssignmentFormulation.pricing_mode
= :exact` vs. `:darp_modified`) must converge to the same master objective.** This is
...
independent of when each was reached" -- `darp_modified/` computes that same optimum
...
physically unrepresentable as *any* `darp_modified/` route, whenever a passenger's
...
`darp_modified/` cannot avoid taking the worse credit, or must pay extra travel to
...
export JointRoutingAssignmentDarpModifiedPricingData
export JointRoutingAssignmentDarpModifiedPricingLabel

struct JointRoutingAssignmentDarpModifiedPricingData
    scenario::Int
    nodes::Vector{Int}
...
"""
struct JointRoutingAssignmentDarpModifiedPricingLabel
    current::Int
    route::Vector{Int}
    time::Float64
...
const JointRoutingAssignmentDarpModifiedLabelId = Int
const JointRoutingAssignmentDarpModifiedLabelOrderKey = Tuple{Float64, Float64, Int, Int}

"""
    JointRoutingAssignmentDarpModifiedAction
...
(`JointRoutingAssignmentDarpModifiedEligibility`, `data.jl`) this particular branch
...
"""
Hot-path mirror of a `JointRoutingAssignmentDarpModifiedPricingLabel`, the same shape
...
"""
struct JointRoutingAssignmentDarpModifiedLabelBitsets
    served_bits::BitSet
...
"""
struct JointRoutingAssignmentDarpModifiedDominanceFilters
    reduced_cost::Float64
    time::Float64
    age_mask::UInt64
    route_length::Int32
...
"""
struct JointRoutingAssignmentDarpModifiedDominanceRules{BoundedStops, Compensated} <: AbstractPricingDominanceRules end

_joint_routing_assignment_darp_modified_dominance_rules(bounded_max_stops::Bool, compensated_dominance::Bool) =
