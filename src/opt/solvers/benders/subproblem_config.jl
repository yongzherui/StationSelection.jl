...
`:column_generation_activated_lpo` prices the same way and then REPLACES the cut's dual point
with a locally Pareto-optimal one, in the sense of Cummings, Jacquillat and Vaze's activated
Benders procedure: keep the activated solve's VALUE `z*`, re-optimise the dual over the
optimal face, and pick the representation that maximises the cut at a core point. The
completion rule (`rho_pjk <= 0` on every assignment touching an unbuilt station) is OURS, not
theirs -- their omitted multipliers reconstruct coordinate-wise, ours are coupled through the
pickup/dropoff reward. `lpo_completion` picks which half of the experiment runs:

- `:pareto` (default) -- the full optimal-face re-optimisation, Method B. Because the second
  stage is column-generated, solving the auxiliary LP once over the pool is NOT sufficient:
  the pool certified the ORIGINAL duals, and this moves them. The implementation therefore
  re-prices after every LP solve and adds each negative-reduced-cost route as a new DUAL ROW,
  until pricing certifies feasibility over the whole activated family.
- `:baseline` -- Method A, the control: keep the activated solve's own `alpha`/built `gamma`
  and choose only the unbuilt multipliers, minimising their damage at the core point under the
  same `rho <= 0` rule. No pricing at all; the `rho <= 0` argument discharges every route row
  outright at these unmoved multipliers.

Both arms use the SAME validity rule, deliberately: comparing a Pareto dual against a baseline
allowed a weaker rule would measure the rule rather than the Pareto step.

`lpo_core_point` selects how the core point is built, and it is NOT a cosmetic choice: the
Pareto objective `C(y^c) = Σα − Σ_j y^c_j g_j` ignores any coordinate with `y^c_j = 0`, so a
core point with a zero coordinate turns the objective into a seminorm and hands `g_j` an
unbounded improving ray. `:relative_interior` (the default) constructs a point with no such
coordinate by averaging per-coordinate and per-row witnesses; `:max_min_slack` maximises the
minimum slack and degenerates to a VERTEX whenever one face is structurally tight (measured:
mean unbuilt `g_j` of 1.8e6 against the closed form's 7.1e3, half the neighbourhood
uninformative); `:uniform` is `y^c_j = k/|J|`, which satisfies the budget row but can sit on
-- or outside -- an endpoint-feasibility row. See `_benders_core_point`. `lpo_max_rounds` caps the Pareto pricing loop,
`lpo_pricing_time_limit_sec` budgets each of its separation rounds, and `lpo_variable_bound`
is a safety box on the auxiliary LP's variables (default: 100x the largest incoming `alpha`)
whose only job is to turn an unbounded ray into a reported `bound_binding` status. A
truncated, rejected or bound-limited loop installs the caller's closed-form completion
unchanged -- so it degrades to `:column_generation_activated`'s cut and can never produce an
invalid one. The full derivation, including why the `rho <= 0` rows keep every separation
round ACTIVATED (which is what distinguishes this from the row-generation completion removed
on 2026-09-10), lives with the code in
`optimize/aggregate_od_route/benders/completion_lpo.jl`.
...
# The activated subproblem's dual completion

The restriction and the dual repair are the SAME operation: raising `gamma` to `alpha_p` on
an unbuilt station drives every candidate through it to `rho <= 0` and out of the pricer's
own filter, so the exhaustion the inner loop certifies is already full-universe at those
duals. The completion is not a repair applied afterwards.

The validity proof, the shortcut argument it rests on, and the measured cut strength live
with the code they constrain -- `_benders_activated_complete_duals!` in
`optimize/aggregate_od_route/benders/subproblem.jl`. They used to sit here instead, which is
how a false premise in that proof survived the pricer changes that invalidated it: a proof
parked away from the code it constrains is one nobody re-checks when that code moves.
...
struct BendersSubproblemConfig
    ...
    lpo_core_point::Symbol
    lpo_max_rounds::Int
    lpo_pricing_time_limit_sec::Float64
    lpo_variable_bound::Union{Nothing, Float64}
    verbose::Bool
    ...
    function BendersSubproblemConfig(;
            ...
            lpo_completion::Symbol=:pareto,
            lpo_core_point::Symbol=:relative_interior,
            lpo_max_rounds::Int=20,
            lpo_pricing_time_limit_sec::Number=300.0,
            lpo_variable_bound::Union{Nothing, Number}=nothing,
            verbose::Bool=false,
        )
        oracle in (:direct_enumeration, :column_generation,
            ...
            ":column_generation_activated_lpo " *
            "or :column_generation_warm_start",
        ))
        lpo_completion in (:baseline, :pareto) || throw(ArgumentError(
            "unsupported lpo_completion $(repr(lpo_completion)); expected :pareto (the " *
            "optimal-face re-optimisation) or :baseline (the ordinary activated dual with " *
            "a damage-minimising completion, i.e. the control arm)",
        ))
        lpo_core_point in (:relative_interior, :max_min_slack, :uniform) ||
            throw(ArgumentError(
            "unsupported lpo_core_point $(repr(lpo_core_point)); expected " *
            ":relative_interior (the default -- averaged witnesses, no zero coordinate), " *
            ":max_min_slack (max-min slack, which DEGENERATES to a vertex when some face is " *
            "structurally tight) or :uniform (y^c_j = k/|J|, not in general feasible)",
        ))
        ...
        isnothing(lpo_variable_bound) || lpo_variable_bound > 0 ||
            throw(ArgumentError("lpo_variable_bound must be positive"))
        ...
        new(
            ...
            lpo_completion, lpo_core_point, lpo_max_rounds,
            Float64(lpo_pricing_time_limit_sec),
            isnothing(lpo_variable_bound) ? nothing : Float64(lpo_variable_bound),
            verbose,
        )
    end
end
