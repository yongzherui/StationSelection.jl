...
Add sparse binary assignment variables x[s][t_id][od_idx] for TwoStageRouteWithTimeModel.
...
"""
    add_assignment_variables!(m::Model, data::StationSelectionData, mapping::RouteODMap)

Add sparse binary assignment variables x[s][od_idx] for RouteAlphaCapacityModel /
RouteVehicleCapacityModel.

For each OD pair (o,d) in scenario s, one binary variable is created per valid (j,k) pair.
