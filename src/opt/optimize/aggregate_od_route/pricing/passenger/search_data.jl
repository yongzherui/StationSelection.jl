...
`PassengerFreeAssignmentPricingData` is the semantic pricing input. The types in
...
struct PassengerFreeAssignmentSearchIndex
...
function _build_passenger_free_assignment_search_index(
    ...
    return PassengerFreeAssignmentSearchIndex(
        node_index, travel_matrix, opp_dest_idx, opp_ride_limit, opp_layer_mask,
        opps_by_origin_idx, origin_union_mask, nodes_by_travel,
    )
end

function _create_passenger_free_assignment_bound_workspace(n_nodes::Int)
    return PassengerFreeAssignmentBoundWorkspace(
