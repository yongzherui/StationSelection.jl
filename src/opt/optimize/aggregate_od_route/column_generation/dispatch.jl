...
# `CGSolver.pricing.warm_start_mode` support. Only the joint formulation has a selectable
...
# `pricing_mode = :relaxed_cluster` support -- the relaxed-cluster round, which is a
# pricer AND the only search that can certify. Structurally parallel to the warm-start pair
# above, but it answers both questions at once: which columns to add, and whether any
# remain at all. Only the joint formulation has a relaxation
...
# from a `CGPricingConfig` with no `relaxed_cluster_count`, falls through to
# `cg_certification_supported`'s `false` default, so asking for the mode is rejected rather
