...
function _base_aggregate_od_route_model(model::AnyAggregateODRouteProblem)::AggregateODRouteProblem
    return model isa AggregateODRouteProblem ? model : model.base
end
...
function _check_aggregate_od_route_endpoint_feasibility!(
    ...
    y = m[:y]
    add_station_limit_constraint!(m, data, base.l)
    _add_default_endpoint_coverage_constraints!(m, y, data, model, requests)
    optimize!(m)
    primal_status(m) == MOI.FEASIBLE_POINT || throw(ArgumentError(
        "AggregateODRouteProblem Benders pre-flight check failed: no y with sum(y)==$(base.l) can open a " *
...
main `AggregateODRouteProblem` master's one-per-`(j, k, s)` aggregated coverage row. A new
