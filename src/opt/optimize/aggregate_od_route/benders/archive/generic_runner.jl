...
over `AggregateODRouteProblem`. The loop shape (solve master, derive priming assignments,
...
function _benders_tighten_subproblem_value(
    ...
    subproblem_model::AggregateODRouteProblem,
    ::D,
...
function _run_benders_decomposition(
    ...
    if model.assignment_policy isa NearestOpenAggregateODAssignmentPolicy &&
       model.assignment_policy.feasibility_cut_style == :pair_chain
        assert_no_walk_only_pairs(mapping, "AggregateODRouteProblem Benders ($(decomposition_name), NearestOpen, :pair_chain)")
    end
