"""
Top-level `run_opt` dispatch for `BendersSolver` over `AggregateODRouteProblem`, routing
...
function run_opt(
    ...
    solver.decomposition isa BendersY &&
        throw(ArgumentError("AggregateODRouteProblem free assignment Benders supports BendersXY only; BendersY is unsupported"))
    solver.decomposition isa BendersXY &&
