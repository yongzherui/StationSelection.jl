"""
Heuristic-enumeration warm start for AggregateODRouteProblem.
...
candidate by routing objective is folded into the full AggregateODRouteProblem's column pool
...
`AggregateODRouteProblem`/`RouteCoveringProblem` JuMP model. Unlike `_apply_warm_start!` in
...
function run_opt(
    ...
    isempty(requests) &&
        throw(ArgumentError("AggregateODRouteProblem heuristic enumeration requires positive demand"))
