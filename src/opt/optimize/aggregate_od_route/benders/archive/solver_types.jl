...
Benders decomposition (`AggregateODRouteProblem`, `NearestOpenAggregateODAssignmentPolicy`
...
`AggregateODRouteProblem`'s `NearestOpenAggregateODAssignmentPolicy` and
...
Solve `AggregateODRouteProblem` by trying a caller-supplied list of candidate open-station
...
then used to warm-start a direct solve of the full `AggregateODRouteProblem` (with the
...
`AggregateODRouteProblem`'s `FreeAggregateODAssignmentPolicy`.
