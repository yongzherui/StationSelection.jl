...
function build_model(
    ...
    if !isnothing(model.max_active_per_zone)
        model.max_active_per_zone * mapping.n_clusters >= model.k ||
            throw(ArgumentError(
                "max_active_per_zone ($(model.max_active_per_zone)) * resolved " *
                "n_clusters ($(mapping.n_clusters)) < k ($(model.k)): infeasible. " *
                "Increase max_active_per_zone or reduce k."
            ))
    end
    ...
    if !isnothing(model.max_active_per_zone)
        constraint_counts["zone_activation_limit"] = add_zone_activation_limit_constraints!(
            m, data, mapping, model.max_active_per_zone
        )
    end
