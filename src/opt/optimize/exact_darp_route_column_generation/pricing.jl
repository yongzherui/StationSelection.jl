...
export solve_exact_darp_route_pricing

"""
Exact bucketed pricing for the current ExactDARPRoute column-generation path.
...
function _cg_log(message::AbstractString; kwargs...)
    parts = ["[exact_darp_route_cg] $message"]
...
function _column_log_summary(column::ExactDARPRoutePricedColumn)
...
function compute_qbar(
    ...
    time_window_sec::Int
)::Dict{Int, ExactDARPRouteBucketDemandCaps}
    ...
    return Dict(
        t_id => ExactDARPRouteBucketDemandCaps(0, t_id, caps)
        for (t_id, caps) in by_bucket
    )
end

function _bucket_duals(
    duals::ExactDARPRouteCGDuals,
    scenario_idx::Int,
    time_id::Int
...
function _candidate_station_set(
    qbar::ExactDARPRouteBucketDemandCaps,
...
function _build_bucket_pricing_data(
    ...
    qbar::ExactDARPRouteBucketDemandCaps,
    duals::Dict{Tuple{Int, Int}, Float64},
)::ExactDARPRouteBucketPricingData
    return ExactDARPRouteBucketPricingData(
        scenario_idx,
        time_id,
        _candidate_station_set(qbar, duals),
        ExactDARPRouteBucketDemandCaps(scenario_idx, time_id, copy(qbar.caps)),
        copy(duals),
...
function _label_signature(label::ExactDARPRoutePricingLabel)
    return (
...
function _current_load(label::ExactDARPRoutePricingLabel)::Int
    return sum(values(label.onboard); init=0)
end

function dropoff(label::ExactDARPRoutePricingLabel, station_idx::Int)::Dict{Tuple{Int, Int}, Int}
...
function candidate_next_stations(
    label::ExactDARPRoutePricingLabel,
    pricing_data::ExactDARPRouteBucketPricingData,
    data::StationSelectionData
)::Vector{Int}
...
function enumerate_pickup_vectors(
    label_after_dropoff::ExactDARPRoutePricingLabel,
    station_idx::Int,
    pricing_data::ExactDARPRouteBucketPricingData,
...
function extend_label(
    ...
    pricing_data::ExactDARPRouteBucketPricingData,
    data::StationSelectionData
)::Union{ExactDARPRoutePricingLabel, Nothing}
    ...
    return ExactDARPRoutePricingLabel(
        next_station,
        new_visited,
...
function dominates(label1::ExactDARPRoutePricingLabel, label2::ExactDARPRoutePricingLabel)::Bool
...
function _push_if_nondominated!(
    store::Dict,
    open_stack::Vector{ExactDARPRoutePricingLabel},
    label::ExactDARPRoutePricingLabel,
    dominated_counter::Base.RefValue{Int}
)
    signature = _label_signature(label)
    bucket = get!(store, signature, ExactDARPRoutePricingLabel[])
...
function _route_from_label(
    ...
    return ExactDARPRoutePricedColumn(
        scenario_idx,
        time_id,
        route,
        alpha_profile,
...
function _positive_origin_stations(pricing_data::ExactDARPRouteBucketPricingData)::Vector{Int}
    origins = Set{Int}()
...
function _initial_labels(
...
)::Tuple{Vector{ExactDARPRoutePricingLabel}, Int}
    labels = ExactDARPRoutePricingLabel[]
    initialized = 0
    for start_station in _positive_origin_stations(pricing_data)
        ...
        for pickup_vector in pickup_vectors
            ...
            push!(labels, ExactDARPRoutePricingLabel(
                start_station,
                visited,
                [start_station],
                0.0,
...
function _price_bucket(
    ...
    isempty(initial_labels) && return (
        columns=ExactDARPRoutePricedColumn[],
        status=:optimal,
    ...
    nondominated = Dict{Any, Vector{ExactDARPRoutePricingLabel}}()
    for label in initial_labels
        push!(get!(nondominated, _label_signature(label), ExactDARPRoutePricingLabel[]), label)
    end

    dominated_count = Ref(0)
    expanded_count = 0
    completed_labels = ExactDARPRoutePricingLabel[]
    best_rc = Inf
    t0 = time()
    status = :optimal
    ...
    while !isempty(open_stack)
        ...
        if isempty(label.onboard) && completed_reduced_cost < rc_tolerance && length(label.route_sequence) >= 2
            completed_label = ExactDARPRoutePricingLabel(
                label.current_station,
                label.visited,
        ...
        for next_station in candidate_next_stations(label, pricing_data, data)
            onboard_after_dropoff = dropoff(label, next_station)
            residual_capacity = pricing_data.vehicle_capacity - sum(values(onboard_after_dropoff); init=0)
            after_dropoff = ExactDARPRoutePricingLabel(
                label.current_station,
                label.visited,
                label.route_sequence,
                label.resource_tau,
...
function price_scenario(
    ...
    qbar_by_bucket::Dict{Int, ExactDARPRouteBucketDemandCaps};
    rc_tolerance::Float64=-1e-6,
    max_columns::Int=10,
    time_limit_sec::Float64=60.0,
    return_all_negative::Bool=false,
)::ExactDARPRoutePricingResult
    all_columns = ExactDARPRoutePricedColumn[]
    ...
    for time_id in sort!(collect(keys(bucket_duals)))
        ...
        qbar_t = get(qbar_by_bucket, time_id, ExactDARPRouteBucketDemandCaps(scenario_idx, time_id, Dict{Tuple{Int, Int}, Int}()))
    ...
    return ExactDARPRoutePricingResult(
        all_columns,
        status,
        join(messages, " | "),
        Dict{String, Any}(
...
function solve_exact_darp_route_pricing(
    model::ExactDARPRouteModel,
    data::StationSelectionData,
    ...
    scenario_results = ExactDARPRoutePricingResult[]
    all_columns = ExactDARPRoutePricedColumn[]
    metadata = Dict{String, Any}()
    status = :optimal
    ...
    novel_columns = ExactDARPRoutePricedColumn[]
