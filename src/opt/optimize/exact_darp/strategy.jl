export ExactDARPRouteIterativeStrategy

struct ExactDARPRouteIterativeStrategy <: AbstractIterativeSolveStrategy
    config::ExactDARPRouteRunnerConfig
end

function _exact_darp_route_pool_target_length(strategy::ExactDARPRouteIterativeStrategy)::Union{Int, Nothing}
...
function initialize_iteration_state(
    strategy::ExactDARPRouteIterativeStrategy,
    ...
    state = initialize_route_pool(
        ...
        initial_generated_max_route_length=_exact_darp_route_pool_target_length(strategy)
    )
...
function run_iteration_subproblem(
    ...
    model::ExactDARPRouteModel,
    data::StationSelectionData,
    state::ExactDARPRouteBucketPoolsState;
...
iteration_state_size(strategy::ExactDARPRouteIterativeStrategy, state::ExactDARPRouteBucketPoolsState)::Int =
    sum(length(bucket.routes_by_id) for bucket in values(state.bucket_states))

function _bucket_count(state::ExactDARPRouteBucketPoolsState)::Int
...
function update_iteration_state!(
    ...
    @info "exact_darp_route iteration start" iteration=iteration pool_before=pool_before prune_enabled=strategy.config.prune_enabled expand_enabled=strategy.config.expand_enabled enrichment_enabled=strategy.config.enrichment.enabled
    ...
    for bucket_key in _sorted_bucket_route_pool_keys(state)
        ...
        if strategy.config.prune_enabled
            ...
            removed > 0 && @debug "exact_darp_route bucket prune" bucket=bucket_key removed=removed bucket_target_size=bucket_target_size active_routes=count(v -> v > strategy.config.min_theta_to_keep, values(bucket_usage))
        end
    end
    ...
    if strategy.config.prune_enabled
        ...
        cap_result.removed > 0 && @debug "exact_darp_route global prune" removed=cap_result.removed buckets_touched=cap_result.buckets_touched route_pool_target_size=strategy.config.route_pool_target_size
    end
    ...
    if strategy.config.expand_enabled && !isempty(strategy.config.route_length_schedule)
        ...
        for bucket_key in _sorted_bucket_route_pool_keys(state)
            ...
            exp.added > 0 && @debug "exact_darp_route bucket expansion" bucket=bucket_key target_length=target_length added=exp.added added_by_strategy=exp.added_by_strategy alpha_entries_added=exp.n_alpha n_seeds=exp.n_seeds n_insertion_iters=exp.n_iters
        end
        total_added += exp_added
        @info "exact_darp_route iteration expansion" iteration=iteration target_length=target_length expanded_buckets=exp_buckets routes_added=exp_added expanded_by_strategy=exp_by_strategy alpha_entries_added=exp_alpha total_seeds=exp_seeds total_insertion_iters=exp_iters
    end

    pool_after = sum(length(b.routes_by_id) for b in values(state.bucket_states))
    @info "exact_darp_route iteration summary" iteration=iteration pool_before=pool_before pool_after=pool_after pruned_routes=total_removed pruned_buckets=total_pruned_buckets enriched_profiles=enrichment_info.added enriched_buckets=get(enrichment_info, :n_buckets_with_pressure, 0) expanded_routes=exp_added expanded_buckets=exp_buckets expanded_by_strategy=exp_by_strategy active_routes=total_active n_pressured_legs=get(enrichment_info, :n_pressured_legs, 0) n_binding_legs=get(enrichment_info, :n_binding_legs, 0)
...
function finalize_iterative_result!(
    ...
    history::Vector{IterativeSolveIterationSummary},
    final_state::ExactDARPRouteBucketPoolsState,
    ...
    final_result.metadata["exact_darp_route_runner"] = Dict(
        "iterations" => [
            Dict(
