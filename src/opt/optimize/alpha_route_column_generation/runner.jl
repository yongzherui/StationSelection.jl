...
function run_alpha_route_column_generation(
    ...
    if do_optimize && lp_final_result.termination_status == MOI.OPTIMAL
        @info "alpha_route_cg: solving final integer master" route_count=sum(length(bucket.routes_by_id) for bucket in values(iterative_result.final_state.route_pool.bucket_states))
        ...
        cg_metadata = get(lp_final_result.metadata, "alpha_route_column_generation", Dict{String, Any}())
        ...
        integer_result.metadata["alpha_route_column_generation"] = cg_metadata
        ...
        @info "alpha_route_cg: final integer master complete" termination_status=integer_result.termination_status lp_objective=lp_objective integer_objective=integer_objective lp_gap=cg_metadata["lp_relaxation_gap"]
    end
