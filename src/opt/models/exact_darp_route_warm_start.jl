"""
ExactDARPRouteWarmStartModel — warm start model for ExactDARPRouteModel.
...
"""
    ExactDARPRouteWarmStartModel <: AbstractODModel

Warm start model for ExactDARPRouteModel. Solves only for y/z/x with a walking cost
...
"""
struct ExactDARPRouteWarmStartModel <: AbstractODModel
    k                    :: Int
    l                    :: Int
    max_walking_distance :: Float64
    time_window_sec      :: Int

    function ExactDARPRouteWarmStartModel(
            k::Int,
            l::Int;
...
"""
    create_map(model::ExactDARPRouteWarmStartModel, data) -> ExactDARPRouteODMap
...
"""
function create_map(
    model :: ExactDARPRouteWarmStartModel,
...
"""
    build_model(model::ExactDARPRouteWarmStartModel, data; optimizer_env=nothing) -> BuildResult
...
"""
    get_exact_darp_route_warm_start_solution(model::ExactDARPRouteModel, data, build_result;
                            optimizer_env=nothing, silent=true, kwargs...)
                            -> Union{Dict{Symbol,Any}, Nothing}

Solve ExactDARPRouteWarmStartModel (y/z/x only, no routes) to get a fast incumbent
...
since alpha is a fixed parameter in ExactDARPRouteModel.
...
function get_exact_darp_route_warm_start_solution(
    ...
    ws_model = ExactDARPRouteWarmStartModel(
        model.k, model.l;
    ...
    if !has_values(ws_m)
        @warn "get_exact_darp_route_warm_start_solution: no feasible solution found" termination_status=string(ts)
        return nothing
    end
...
function _derive_theta_hints_arm(
    x_vals       :: Vector,
    ws_mapping   :: ExactDARPRouteODMap,
    main_mapping :: ExactDARPRouteODMap
)
    S = length(x_vals)
...
Evaluate the ExactDARPRouteModel covering constraint against warm start hint values:
