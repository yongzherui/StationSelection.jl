...
Uses the total-demand-cap uncertainty set:
    U_s = { q_s : q̲_ods ≤ q_ods ≤ q̄_ods ∀(o,d),  Σ_{od} q_ods ≤ Q̄_s }
...
- `q_low`: lower demand bounds q̲_ods; `q_low[s][(o,d)]`
- `q_hat`: demand range q̂_ods = q̄_ods − q̲_ods; `q_hat[s][(o,d)]`
- `B`: per-scenario budget B_s = Q̄_s − Σ_{od} q̲_ods; `B[s]`

Demand-bound data is pre-calibrated externally (see `compute_demand_bounds`).
The scenario indices in q_low/q_hat/B must align with the ScenarioData order
in the StationSelectionData passed to `run_opt`.

# Mathematical Formulation (robust counterpart)
    min  Σ_{s,od} q̲_ods t_ods + Σ_s B_s α_s + Σ_{s,od} q̂_ods β_ods
