...
`k` (stations built) and `max_walking_distance` come from `problem`, not from these
structs -- the redundant per-formulation `k`/`max_walking_distance` fields from the
...
- `ClusteringBaseFormulation`: single-scenario k-medoids, station-to-station assignment.
  No build/activate split, so `problem.k` *is* the station count selected directly --
...
- `ClusteringTwoStageFormulation`: two-stage (build/activate) station-to-station
  assignment. Carries `l` (activate per scenario); `k` (build) is `problem.k`.
- `ClusteringTwoStageODFormulation`: two-stage (build/activate) OD pickup/dropoff
  assignment. Carries `l`, `in_vehicle_time_weight`.
- `ClusteringTwoStageODFlowRegularizerFormulation`: `ClusteringTwoStageODFormulation` +
  a route-activation flow-regularization penalty. Carries `l`, `in_vehicle_time_weight`,
  `flow_regularization_weight`.

`l <= problem.k` can no longer be validated at formulation-construction time (no `k` in
...
Basic k-medoids clustering formulation for station selection: selects `problem.k`
...
so there is no formulation-specific knob left once `k`/`max_walking_distance` moved to
...
- Select exactly `problem.k` stations
"""
...
# Fields
- `l::Int`: Number of stations to activate per scenario. Stations built (`k`) comes
  from `StationSelectionProblem.k`.
...
struct ClusteringTwoStageFormulation <: AbstractClusteringFormulation
    ...
    function ClusteringTwoStageFormulation(l::Int)
...
# Mathematical Formulation
First stage: Select k stations to build (y[j] ∈ {0,1})
Second stage: For each scenario s, activate l stations (z[j,s] ∈ {0,1})
...
Constraints:
- Σ_j y[j] = k                              (build exactly k stations)
- Σ_j z[j,s] = l  ∀s                        (activate l stations per scenario)
...
struct ClusteringTwoStageODFormulation <: AbstractClusteringTwoStageODFormulation
    ...
    function ClusteringTwoStageODFormulation(
        ...
        new(l, Float64(in_vehicle_time_weight))
    end
end
...
# Fields
- `l`, `in_vehicle_time_weight`: see `ClusteringTwoStageODFormulation`
...
struct ClusteringTwoStageODFlowRegularizerFormulation <: AbstractClusteringTwoStageODFormulation
    ...
    function ClusteringTwoStageODFlowRegularizerFormulation(
        ...
        new(l, Float64(in_vehicle_time_weight), Float64(flow_regularization_weight))
    end
end
