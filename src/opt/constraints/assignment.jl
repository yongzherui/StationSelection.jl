...
# ============================================================================
# TwoStageRouteODMap (TwoStageRouteWithTimeModel)
# ============================================================================

"""
    add_assignment_constraints!(m, data, mapping::TwoStageRouteODMap)

Each OD pair must be assigned to exactly one valid (j,k) pair.
    Σ x[s][t_id][od_idx] == 1  ∀(s, t_id, od_idx)
"""
function add_assignment_constraints!(
        m::Model,
        data::StationSelectionData,
    ...
    for s in 1:S
        for (t_id, od_pairs) in mapping.Omega_s_t[s]
            for od_idx in 1:length(od_pairs)
...
"""
    add_assignment_to_active_constraints!(m, data, mapping::TwoStageRouteODMap;
                                          tight_constraints=true)

Assignments require both pickup and dropoff stations to be active.
Tight form (default): x[...][pair_idx] ≤ z[j,s]  AND  x[...][pair_idx] ≤ z[k,s]
"""
function add_assignment_to_active_constraints!(
        m::Model,
        data::StationSelectionData,
...
# ============================================================================
# RouteODMap (RouteAlphaCapacityModel / RouteVehicleCapacityModel)
# ============================================================================

"""
    add_assignment_constraints!(m, data, mapping::RouteODMap)
...
"""
    add_assignment_to_active_constraints!(m, data, mapping::RouteODMap; tight_constraints=true)
