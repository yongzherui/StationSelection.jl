"""
`:big_m_nearest`/`:endpoint_chain` nearest-open styles: a per-physical-endpoint `z`
selector chain (cascading domination rows for `:endpoint_chain`, a perturbed Big-M row for
`:big_m_nearest`), shared by the compact-model build, `BendersXY`'s master, and the
`BendersY`/`BendersYZ`/`BendersYZH` LP subproblems/masters. The two styles share nearly all
of this machinery -- only variable/row construction (`_endpoint_chain_variable!` vs.
`_endpoint_big_m_variable!`) differs, dispatched by `selector_style` in
`_endpoint_selector_variable!` -- so they live in one file rather than two.
"""
