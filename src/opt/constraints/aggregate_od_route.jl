...
const AggregateODRouteEndpointChainKey = Tuple{Symbol, Tuple{Int, Vararg{Int}}, Tuple{Float64, Vararg{Float64}}}
const _AggregateODRouteEndpointChainKey = AggregateODRouteEndpointChainKey
...
"""
    _add_endpoint_open_feasibility_cut!(master, y, candidates) -> ConstraintRef

`sum(y[j] for j in candidates) >= 1` -- at least one candidate station for one
physical endpoint must be open. Building block of
[`_add_default_endpoint_coverage_constraints!`](@ref) and the reactive
feasibility-cut fallback in the Benders outer loops.
"""
...
in the identical shape `add_aggregate_od_route_coverage_constraints!` uses (5-tuple key
`(j, k, s, od_idx, pair_idx)`, `pair_idx` = index into that OD bucket's `get_valid_jk_pairs`
list, coinciding with the pair's rank here since every feasible pair gets exactly one row),
so `extract_aggregate_od_route_coverage_duals` and the rest of the pricing/column-generation
pipeline work completely unmodified — the `σ` reward aggregation pricing needs is already
implemented there; it now just sums duals of these new rows instead of the old ones.
