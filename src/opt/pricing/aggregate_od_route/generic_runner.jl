...
formulation) and `PassengerFreeAssignmentCG` (`pricing/passenger/column_generation.jl`, the
...
(`run_aggregate_od_route_column_generation`, `run_passenger_free_assignment_column_generation`)
...
`PassengerFreeAssignmentCG` (Phase 5) overrides this to a large safety constant, since its own
...
`PassengerFreeAssignmentCG` (Phase 5) resolve this sub-case identically; algorithm-specific extra
...
"""
    _cg_build_master(data, formulation::AnyAggregateODRouteModel, algorithm::AggregateODRouteCG, solver, optimizer_env)
