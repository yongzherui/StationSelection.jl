"""
Single dispatch choke point for `ColumnGenerationSolver` over `AggregateODRouteModel`/
...
`FreeAggregateODAssignmentPolicy` models route to `PassengerFreeAssignmentCG`; every other
`AggregateODRouteModel` and every `RouteCoveringProblem` (whose assignments are already fixed, so
passenger-level free assignment never applies) route to `AggregateODRouteCG`.
"""
_default_cg_algorithm(formulation::AggregateODRouteModel) =
    formulation.assignment_policy isa FreeAggregateODAssignmentPolicy ?
        PassengerFreeAssignmentCG() : AggregateODRouteCG()
...
function run_opt(
    ...
    if algorithm isa PassengerFreeAssignmentCG
        formulation.assignment_policy isa FreeAggregateODAssignmentPolicy || throw(ArgumentError(
            "PassengerFreeAssignmentCG requires FreeAggregateODAssignmentPolicy; got " *
    ...
    elseif algorithm isa AggregateODRouteCG
        formulation.assignment_policy isa FreeAggregateODAssignmentPolicy && throw(ArgumentError(
            ...
            "PassengerFreeAssignmentCG() instead"
        ))
