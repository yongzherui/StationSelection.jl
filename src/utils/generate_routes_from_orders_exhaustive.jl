...
Each generated route's `od_capacity` field records, for every (j,k) pair where j
appears before k in the station sequence, the vehicle capacity if the in-vehicle
detour for that leg satisfies the detour constraints — and omits the key otherwise.
The MILP uses this to suppress `alpha_r_jkts` variables for infeasible legs.
"""

export generate_simple_routes_exhaustive


"""
    generate_simple_routes_exhaustive(
...
For each recorded route, `od_capacity[(j_id, k_id)]` is set to `vehicle_capacity` for
every (j,k) leg whose in-vehicle detour satisfies `max_detour_time` and
`max_detour_ratio`; infeasible legs are omitted (effectively capacity 0).
...
5. When the route has length ≥ 2 and all positions are covered, record it with
   detour-filtered `od_capacity`.
"""
function generate_simple_routes_exhaustive(
    valid_jk_pairs          :: Set{Tuple{Int,Int}},
    array_idx_to_station_id :: Vector{Int},
    data                    :: StationSelectionData;
    max_route_length        :: Int     = 4,
    ...
    function dfs!(covered :: BitSet)
        ...
        if m >= 2 && length(covered) == m
            ...
            if !haskey(routes_map, sids)
                ...
                tt = sum(seg; init = 0.0)
                # Build od_capacity: include (i,j) leg only if detour is feasible
                    ...
                    for j in (i + 1):m
                        ...
                        if (cum - direct <= max_detour_time) &&
                           (direct == 0.0 || cum / direct <= 1.0 + max_detour_ratio)
                            od_cap[(sids[i], sids[j])] = vehicle_capacity
                        end
                    end
                end
                next_id[] += 1
                routes_map[sids] = RouteData(next_id[], sids, tt, od_cap)
            end
        end
